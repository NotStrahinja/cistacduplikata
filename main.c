...
void scan_directory(const char *base_path, BOOL recursive, BOOL useExtensions)
{
    ...
    do {
        ...
        {
            if(recursive) scan_directory(full_path, recursive, useExtensions);
            else continue;
        }
        else
        {
            if(useExtensions)
            {
                const char *file_ext = get_file_ext(findData.cFileName);
                if(!extension_allowed(file_ext))
                    continue;
            }
        ...
        switch(uMsg)
        {
            case WM_COMMAND:
                ...
                if(LOWORD(wParam) == 2 && HIWORD(wParam) == BN_CLICKED)
                {
                    ...
                    scan_directory(pathStr, recursive, ext);
                        ...
                        for(int j = i + 1; j < file_count; ++j)
                        {
                            ...
                            if(hashes_equal(files[i].hash, files[j].hash))
                            {
                                ...
                                else
                                {
                                    ...
                                    fprintf(log, "Nemoguce obrisati: %s (Error %lu)\n", files[j].path, err);
                                }
                                free(files[j].path);
                                files[j].path = NULL;
                            }
                ...
                if(HIWORD(wParam) == 0)
                {
                    switch(LOWORD(wParam))
                    {
                        case 4:
                            ...
                            hGledajSF = CreateWindowExA(0, "BUTTON", "Gledaj subfoldere", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_AUTOCHECKBOX, 10, 10, 145, 30, hSettingsWnd, (HMENU)3, GetModuleHandleA(NULL), NULL);
                            hKoristiEx = CreateWindowExA(0, "BUTTON", "Koristi ekstenzije", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_AUTOCHECKBOX, 10, 40, 145, 30, hSettingsWnd, (HMENU)7, GetModuleHandleA(NULL), NULL);
